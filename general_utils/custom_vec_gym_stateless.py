...
class CustomVecEnvStateless(VecEnv):
    """
    ...
    def get_post_data(self,step_type=0):
        ...
        for env in self.envs:
            obs, action_mask = env.get_post_data(step_type=step_type)
    ...
    def reset(self, seed=None, options=None,step_type=0):
        ...
        infos = []
        for env in self.envs:
            obs, action_mask, info = env.reset(seed=seed, options=options,step_type=step_type)
            observations.append(obs)
            action_masks.append(action_mask)
            infos.append(info)
        observations = np.array(observations)
        ...
        return observations, action_masks, aggregated_info
    ...
    def step_wait(self,step_type=0):
        ...
        for i, env in enumerate(self.envs):
            ...
            obs, action_mask, reward, terminated, info = env.step(self.actions[i],step_type=step_type)
            observations.append(obs)
            action_masks.append(action_mask)
        ...
        return observations, action_masks, rewards, terminateds, aggregated_info
