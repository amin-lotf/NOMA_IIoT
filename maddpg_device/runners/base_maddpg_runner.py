...
class Runner(object):
    ...
    def __init__(self, envs):
        ...
        self.maddpg_offloading_config = self.env.maddpg_device_offloading_config
        self.maddpg_tx_config = self.env.maddpg_device_tx_config
        if self.testing_env:
        ...
        if self.maddpg_offloading_config.load_pretrained_weights:
        ...
        self.tx_policy = VectorizedAttentionMADDPGPolicy(self.maddpg_tx_config,
                                                         ...
                                                         device=self.device,
                                                         metadata=self.env.metadata)
        if self.maddpg_tx_config.load_pretrained_weights:
