...
class FixedNormal(torch.distributions.Normal):
    ...
    def log_probs(self, actions):
        ...
        if self.mask is not None:
            ...
            log_prob = super().log_prob(raw_action)*self.mask
            log_prob = log_prob.sum(dim=-1, keepdim=True)
            log_prob -= torch.sum(torch.log(self.action_scale * (1 - torch.tanh(raw_action).pow(2)) + 1e-6)*self.mask, dim=-1,
                                  keepdim=True)
...
class DiagGaussian(nn.Module):
    ...
    def forward(self, x,available_actions=None):
        ...
        return FixedNormal(action_mean, action_logstd.exp(),self.action_scale,self.action_bias,mask=available_actions)
