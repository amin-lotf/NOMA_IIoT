...
for m = 1:length(fl_metrics)
    ...
    for a = 1:length(algo_list)
        ...
        for i = 1:length(number_of_bs)
            ...
            for j = 1:n_runs
                filename = sprintf('%s_deadline_0%d_best%d%s.mat', algo_list{a}, i*5, j, fl_metrics(m));
    ...
    xticks(number_of_bs); %
    % Set labels, title, and legend with desired font size.
    xticklabels({'5','10','15','20','25'}); % explicitly label the ticks
    xlabel("\boldmath $y$", 'Interpreter', 'latex', 'FontSize', font_size_print);
