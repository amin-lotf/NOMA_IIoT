...
for type_idx=1:size(metrics_type,1)
    ...
    for i=1:size(metrics_list,2)
        ...
        if with_marker
            ...
            min_size=min(min_size,max_x_size);
        end
        ...
        if with_mean
            ...
            if with_marker
                ...
                plot(indices, mean_data(indices),'Color',  colors(1),'LineStyle',lines(1), 'Linewidth', line_size_print)
                hold on;
            end
        end
        % l_names=cat(1,legend_names(2:num_runs+1),legend_names(1));
        legend(l_names,'Interpreter','latex','Location','northoutside','FontSize', font_size_outer,'NumColumns',3);
