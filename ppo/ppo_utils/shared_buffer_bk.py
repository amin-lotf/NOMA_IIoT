...
class SharedReplayBuffer(object):
    def __init__(self, args,n_agents, obs_space, state_space, act_space, device,shift_reward=0):
        ...
        if act_space_type == 'Discrete' or act_space_type == 'MultiDiscrete':
            ...
            self.actions = np.zeros((self.episode_length, self.n_envs, self.n_agents, act_shape), dtype=np.int64)
            # Available actions: [offloading_episode_length+1, n_envs, n_agents, act_shape]
            self.available_actions = torch.ones((self.episode_length + 1, self.n_envs, self.n_agents, act_shape),
            ...
            self.actions = np.zeros((self.episode_length, self.n_envs, self.n_agents, act_shape), dtype=np.float32)
            self.available_actions = torch.ones((self.episode_length + 1, self.n_envs, self.n_agents, act_shape),
                                                dtype=torch.int32, device=self.device)
            self.action_log_probs = torch.zeros((self.episode_length, self.n_envs, self.n_agents, 1),
                                                dtype=torch.float32, device=self.device)
            ...
            self.actions = np.zeros((self.episode_length, self.n_envs, self.n_agents, act_shape), dtype=np.float32)
            self.available_actions = torch.ones((self.episode_length + 1, self.n_envs, self.n_agents, len(act_space), act_space[0].shape),
                                                dtype=torch.int32, device=self.device)
            self.action_log_probs = torch.zeros((self.episode_length, self.n_envs, self.n_agents, 1),
                                                dtype=torch.float32, device=self.device)
        else:
        ...
        self.rewards = np.zeros((self.episode_length, self.n_envs, self.n_agents, 1), dtype=np.float32)
    ...
    def insert(self,
        ...
        if available_actions is not None:
            if torch.is_tensor(available_actions):
                ...
                self.available_actions[self.step + 1].copy_(torch.as_tensor(available_actions).to(self.device))

        self.action_log_probs[self.step].copy_(action_log_probs)
        self.value_preds[self.step].copy_(value_preds)

        if torch.is_tensor(rewards):
            self.rewards[self.step-self.shift_reward] = rewards.cpu().numpy()
        else:
            self.rewards[self.step-self.shift_reward] = rewards

        self.masks[self.step + 1].copy_(torch.as_tensor(masks).to(self.device))
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        rewards = torch.from_numpy(self.rewards).to(self.device)  # [T, n_envs, n_agents, 1]
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        obs = self.obs[:-1].reshape(T * N * A, *self.obs.shape[3:])
        state = self.state[:-1].reshape(T * N * A, *self.state.shape[3:])
        rnn_states = self.rnn_states[:-1].reshape(T * N * A, *self.rnn_states.shape[4:])
        rnn_states_critic = self.rnn_states_critic[:-1].reshape(T * N * A, *self.rnn_states_critic.shape[4:])
        actions = torch.from_numpy(self.actions).to(self.device).reshape(T * N * A, *self.actions.shape[3:])
        available_actions = self.available_actions[:-1].reshape(T * N * A, *self.available_actions.shape[3:])
        action_log_probs = self.action_log_probs.reshape(T * N * A, *self.action_log_probs.shape[3:])
        value_preds = self.value_preds[:-1].reshape(T * N * A, *self.value_preds.shape[3:])
        returns = self.returns[:-1].reshape(T * N * A, *self.returns.shape[3:])
        masks = self.masks[:-1].reshape(T * N * A, *self.masks.shape[3:])
        if advantages is not None:
            advantages = advantages.reshape(T * N * A, 1)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        obs = self.obs[:-1].reshape(T * N * A, *self.obs.shape[3:])
        state = self.state[:-1].reshape(T * N * A, *self.state.shape[3:])
        rnn_states = self.rnn_states[:-1].reshape(T * N * A, self.recurrent_n, self.hidden_size)
        rnn_states_critic = self.rnn_states_critic[:-1].reshape(T * N * A, self.recurrent_n, self.hidden_size)
        actions = torch.from_numpy(self.actions).to(self.device).reshape(T * N * A, *self.actions.shape[3:])
        action_log_probs = self.action_log_probs.reshape(T * N * A, *self.action_log_probs.shape[3:])
        value_preds = self.value_preds[:-1].reshape(T * N * A, *self.value_preds.shape[3:])
        returns = self.returns[:-1].reshape(T * N * A, *self.returns.shape[3:])
