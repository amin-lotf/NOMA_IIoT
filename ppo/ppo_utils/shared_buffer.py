...
class SharedReplayBuffer(object):
        ...
        if act_space_type == 'Discrete' or act_space_type == 'MultiDiscrete':
            ...
            if type(available_act_shape) is tuple:
                self.available_actions = torch.ones(
                    (self.episode_length + 1, self.n_envs, self.n_agents, *available_act_shape),
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        actions = torch.from_numpy(self.actions[:shift_adjuster]).to(self.device).reshape(T * N * A, *self.actions.shape[3:])
