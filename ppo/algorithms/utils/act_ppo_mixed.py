        ...
        if self.is_noma:
            ...
            for idx, action_out in enumerate(self.action_outs):
                if idx < self.action_limits[0]:
                    ...
                    mask = action != -1
                    sc_stat[row_indices[mask], action[mask]] += 1
                elif  self.action_limits[0] <= idx < self.action_limits[1]:
                ...
                elif idx == self.action_limits[1]:
                    #MEC power allocation
                    if available_actions is not None:
                        available_acts=available_actions[:, 1, :].unsqueeze(-1).expand(-1,-1,self.n_sbs).flatten(start_dim=1,end_dim=2)
                        action_logits = action_out(x, available_acts)
                        cont_action =  action_logits.sample()
                        cont_action = cont_action * available_acts
                    else:
                        action_logits = action_out(x)
                        cont_action = action_logits.sample()
                    actions.append(cont_action)
                    cont_action_log_probs.append(action_logits.log_probs(cont_action))
                else:
                    # --- Tx Power (Continuous) ---
                    if available_actions is not None:
                        action_logits = action_out(x, available_actions[:, 0,:])
                        cont_action = action_logits.sample()
                        cont_action = cont_action * available_actions[:,0, :]
                    else:
                        action_logits = action_out(x)
                        cont_action = action_logits.sample()
                    actions.append(cont_action)
                    cont_action_log_probs.append(action_logits.log_probs(cont_action))
            disc_action_log_probs = torch.sum(torch.cat(disc_action_log_probs, dim=-1), dim=-1, keepdim=True)
            cont_action_log_probs = torch.sum(torch.cat(cont_action_log_probs, dim=-1), dim=-1, keepdim=True)
            action_log_probs = disc_action_log_probs + cont_action_log_probs
            return torch.cat(actions, -1), action_log_probs, available_actions
        ...
        if self.is_noma:
            ...
            # ----- MULTI-AGENT MODE (Original) -----
                ...
                for idx, (action_out, act) in enumerate(zip(self.action_outs[:self.action_limits[0]], sc_action)):
                    ...
                    action_log_prob = action_logit.log_probs(act)
                    # entropy = action_logit.entropy().mean()
                    entropy_b = action_logit.entropy()
                    if available_actions is not None:
                        action_log_prob = action_log_prob * available_actions[:, 0, idx:idx + 1]
                        entropy_b = entropy_b * available_actions[:, 0, idx]
                    disc_entropy.append(torch.nan_to_num(torch.mean(entropy_b[entropy_b != 0])))
                    disc_action_log_probs.append(action_log_prob)
                    row_indices = torch.arange(sc_stat.shape[0], device=self.device).unsqueeze(-1)
                    mask = orig_act != -1
                    sc_stat[row_indices[mask], orig_act[mask].unsqueeze(-1)] += 1
                disc_action_log_probs = torch.sum(torch.cat(disc_action_log_probs, dim=-1), dim=-1, keepdim=True)
            ...
            # ----- MULTI-AGENT MODE (Original) -----
                ...
                for idx, (action_out, act) in enumerate(zip(self.action_outs[:self.action_limits[0]], offloading_action)):
                    ...
                    if available_actions is not None:
                        ...
                        action_log_prob = action_log_prob * available_actions[:, idx:idx + 1]
                        entropy = entropy * available_actions[:, idx]
                    disc_entropy.append(torch.nan_to_num(torch.mean(entropy[entropy != 0])))
                    disc_action_log_probs.append(action_log_prob)
                    # act = act.cpu().numpy()
            ...
            for idx, (action_out, act) in enumerate(zip(self.action_outs[:self.action_limits[0]], sc_action)):
                ...
                if available_actions is not None:
                    action_log_prob = action_log_prob * available_actions[:, 0, idx:idx + 1]
                    entropy = entropy * available_actions[:, 0, idx]
                disc_entropy.append(torch.nan_to_num(torch.mean(entropy[entropy != 0])))
                disc_action_log_probs.append(action_log_prob)
                row_indices = torch.arange(sc_stat.shape[0], device=self.device).unsqueeze(-1)
                mask = orig_acts != -1
                sc_stat[row_indices[mask], act.unsqueeze(-1)[mask]] += 1
            ...
            for idx, (action_out, act) in enumerate(
                ...
                if available_actions is not None:
                    ...
                    entropy = entropy * available_actions[:, 1, act_idx]
                disc_entropy.append(torch.nan_to_num(torch.mean(entropy[entropy != 0])))
                disc_action_log_probs.append(action_log_prob)
                taken_splits[:, act_idx, :] += act.unsqueeze(-1)
            mec_power_actions = action[:, self.action_limits[1]:self.action_limits[2]]
            if available_actions is not None:
                available_acts = available_actions[:, 1, :].unsqueeze(-1).expand(-1, -1, self.n_sbs).flatten(
                    start_dim=1, end_dim=2)
                action_logits = self.action_outs[-2](x, available_acts)
                cont_action_log_probs.append(action_logits.log_probs(mec_power_actions))
            else:
                action_logits = self.action_outs[-2](x)
                cont_action_log_probs.append(action_logits.log_probs(mec_power_actions))
            cont_entropy = action_logits.entropy().mean()

            tx_power_actions = action[:, self.action_limits[2]:]
            if available_actions is not None:
                action_logits = self.action_outs[-1](x, available_actions[:, 0])
                cont_action_log_probs.append(action_logits.log_probs(tx_power_actions))
            else:
                action_logits = self.action_outs[-1](x)
                cont_action_log_probs.append(action_logits.log_probs(mec_power_actions))
            cont_entropy += action_logits.entropy().mean()
            ...
            disc_entropy_t = torch.stack(disc_entropy)
            disc_entropy = torch.mean(disc_entropy_t[disc_entropy_t != 0])
            log_prob=disc_action_log_probs+cont_action_log_probs
            entropy = disc_entropy + cont_entropy
            return log_prob, entropy
