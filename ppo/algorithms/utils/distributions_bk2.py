...
class FixedNormal(torch.distributions.Normal):
    def __init__(self, loc, scale, mask=None):
        super().__init__(loc, scale)
        self.mask = mask  # shape: [batch_size, num_outputs] or [num_outputs]

    def log_probs(self, actions):
        # Normal log_prob: shape [batch_size, num_outputs]
        lps = super().log_prob(actions)
        if self.mask is not None:
            # multiply by the mask so invalid dims get 0 contribution
            lps = lps * self.mask
        return lps.sum(-1, keepdim=True)
...
class DiagGaussian(nn.Module):
    ...
    def forward(self, x,available_actions=None):
        action_mean = self.fc_mean(x)  # [batch_size, num_outputs]
        zeros = torch.zeros_like(action_mean)
        action_logstd = self.logstd(zeros)  # [batch_size, num_outputs]
        ...
        return FixedNormal(action_mean, action_logstd.exp(),available_actions)
