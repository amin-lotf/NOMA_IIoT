...
class FixedNormal(torch.distributions.Normal):
    def __init__(self,mean,log_std,action_scale,action_bias):
        super(FixedNormal,self).__init__(mean,log_std)
        self.action_scale=action_scale
        self.action_bias=action_bias
    def log_probs(self, actions):
        raw_action = torch.atanh((actions - self.action_bias) / self.action_scale)
    ...
    def sample(self, sample_shape=torch.Size()):
        raw_action = super().sample()  # Reparameterization trick
        action = torch.tanh(raw_action) * self.action_scale + self.action_bias
        # # Log probability of the action
        # log_prob = super().log_prob(raw_action).sum(dim=-1)
