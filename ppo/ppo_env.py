...
class PPOEnv(MECEnvironment):
    def __init__(self, configs):
        ...
        sc_power_action.extend([[0, self.ppo_tx_config.tx_power_quantization - self.ppo_tx_config.quantization_buffer] for _ in range(self.max_users * tx_sbs_per_agent)])
        ...
        joint_offloading_action_space = MultiDiscrete([[0, self.ppo_offloading_config.split_quantization] for _ in range(self.n_sbs*self.max_users*tx_sbs_per_agent)])
    ...
    def get_global_state(self,obs_type):
    ...
    def tx_step(self,action):
        ...
        if not self.continuous_tx_power:
            power_action=(power_action+self.ppo_tx_config.quantization_buffer)/self.ppo_tx_config.tx_power_quantization
        power_action = torch.clamp(power_action, 0.01, 1.0)
    ...
    def offloading_step(self, action):
        ...
        self.task_split_ratio=torch.as_tensor(split_actions,dtype=self.task_split_ratio.dtype,device=self.device)
        idx = self.cur_timeslot % (self.max_task_deadline + 1)
        offloading_decision=action[:,1:self.n_sbs].reshape(self.n_sbs,self.max_users,self.n_sbs-1)/self.ppo_offloading_config.split_quantization
        self.task_offloading_decision[:,:,:-1,idx]=offloading_decision
        rem_split=1-split_actions-offloading_decision.sum(-1)
        self.task_offloading_decision[:, :, -1, idx] = rem_split
        ...
        self.offloading_action_mask=(self.ue_arrival_buffer.reshape((self.global_users,1))>0).expand(-1,self.n_sbs).reshape((self.num_offloading_agents,-1)).cpu().numpy()
