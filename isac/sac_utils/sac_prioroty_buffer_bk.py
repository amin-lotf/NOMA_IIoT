...
class PrioritizedSACBuffer(object):
    ...
    def sample(self, alpha=0.6, beta=0.4):
        ...
        state = self.state.reshape(-1, *self.state.shape[3:])
        next_state = self.next_state.reshape(-1, *self.next_state.shape[3:])
        ...
        next_available_actions = self.next_available_actions.reshape(-1, *self.next_available_actions.shape[3:])
