...
from isac.sac_utils.multi_discrete import MultiDiscrete


class SACEnv(MECEnvironment):
    def __init__(self, configs):
        super(SACEnv, self).__init__(configs)

        self.isac_offloading_config = configs['isac_offloading_config']
        self.isac_tx_config = configs['isac_config']
        self.single_agent=self.isac_tx_config.single_agent
        ...
        self.instant_resource_offload=self.isac_tx_config.instant_resource_offload
        self.joint_decision=self.isac_tx_config.joint_decision
        self.continuous_tx_power=self.isac_tx_config.continuous_tx_power
        # ----- Start  Action Specs----- #
        # We define action_multiplier to deal with single agent (isac) scenario
        tx_sbs_per_agent = self.n_sbs//self.num_tx_agents
        offloading_user_per_agent = self.global_users //self.num_offloading_agents
        action_mask = np.zeros((self.num_tx_agents, 1, tx_sbs_per_agent,self.max_users), dtype=np.int64)
        action_mask[:, :, :,:self.tot_users] = 1
        action_mask=action_mask.reshape((self.num_tx_agents,1,-1))
        self.tx_action_mask = action_mask.repeat(3, axis=1)
        self.default_action_mask = action_mask.squeeze(axis=1)
        ...
        sc_power_action.extend([[0, self.isac_tx_config.tx_power_quantization - self.isac_tx_config.quantization_buffer] for _ in range(self.max_users * tx_sbs_per_agent)])
        ...
        offloading_action_space = MultiDiscrete([[0, self.isac_offloading_config.split_quantization] for _ in range(self.n_sbs*offloading_user_per_agent)])
        joint_offloading_action_space = MultiDiscrete([[0, self.isac_offloading_config.split_quantization] for _ in range(self.n_sbs*self.max_users*tx_sbs_per_agent)])
    ...
    def joint_step(self,action):
        ...
        split_actions = offloading_actions[:,:,0] / self.isac_offloading_config.split_quantization
        ...
        offloading_decision = offloading_actions[:,:,1:].reshape(self.n_sbs, self.max_users,
                                                    self.n_sbs - 1) / self.isac_offloading_config.split_quantization
    ...
    def tx_step(self, action):
        ...
        if not self.continuous_tx_power:
            power_action = (power_action + self.isac_tx_config.quantization_buffer) / self.isac_tx_config.tx_power_quantization
        power_action = torch.clamp(power_action, 0.01, 1.0)
        power_action *= torch.as_tensor(self.tx_action_mask[:, 2]).to(self.device).reshape((self.n_sbs, self.max_users))
        self.chosen_power_ratio = power_action.float()
        # self.cur_timeslot+=1
        self.perform_noma_step()
        noma_metrics = self.metrics_tx
    ...
    def offloading_step(self, action):
        ...
        split_actions = action[:, 0].reshape(
            (self.n_sbs, self.max_users)) / self.isac_offloading_config.split_quantization
        self.task_split_ratio = torch.as_tensor(split_actions, dtype=self.task_split_ratio.dtype, device=self.device)
        idx = self.cur_timeslot % (self.max_task_deadline + 1)
